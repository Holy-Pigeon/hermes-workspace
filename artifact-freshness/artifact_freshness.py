...
# 日期戳文件名正则: 抓 note_2026-06-13_xxx.md 这类**作者按惯例打日期戳**的产出。
# 这类文件的真实新鲜度 = 文件名里的作者日期, 而非 fs mtime——后者会被 git
# checkout/touch/重打包等无关操作刷新, 从而掩盖真实的产线停更(2026-06-29 实证:
# research 全部 note 内容停在 06-13, 但 holder_concentration note 的 mtime 被
# 06-22 一次无关 git commit 刷到 06-21, 致看门狗读到 181h 假新鲜而非真冻结 384h)。
...
def newest_mtime(path, glob=None):
    """返回 path 下最新文件的 mtime(epoch 秒)。文件→自身; 目录→递归最新文件(排除 .git)。
    glob 非空时仅统计**文件名匹配 glob 的文件**(如 note_*.md), 用于隔离
    『自刷新陪衬文件』污染——例如 research/ 目录每夜被自动重建的 INDEX.md 刷新 mtime,
    会永久掩盖真正的 conviction note 停更; 只盯 note_*.md 才能抓到产线静默冻结。
    取不到返回 None(绝不编造时间)。"""
    path = os.path.expanduser(path)
    if not os.path.exists(path):
        return None
    if os.path.isfile(path):
        # 单文件型 heartbeat: glob 对单文件无意义, 直接取其 mtime
        try:
            return os.path.getmtime(path)
        except OSError:
            return None
    # 目录: 递归找最新文件 mtime(可选按 glob 过滤文件名)
    latest = None
...
def newest_filename_date(path, glob=None):
    """返回 path 下匹配 glob 的文件中**文件名日期戳最大**者对应的 epoch 秒(按 UTC 00:00)。
    用于日期戳产线(如 note_YYYY-MM-DD_*.md): 真实新鲜度取作者打的日期, 不被 fs mtime
    被 git/touch 刷新所污染。无任何带日期戳的匹配文件→返回 None(交回退到 mtime)。"""
    path = os.path.expanduser(path)
...
def analyze(now=None):
    ...
    for p in projects:
        ...
        glob = p.get("artifact_glob")  # 可选: 只盯匹配文件名的产出, 隔离自刷新陪衬文件
        date_stamped = p.get("date_stamped")  # 可选: 产出按 note_YYYY-MM-DD 打日期戳→以文件名日期为准
        if not hb:
            continue
        mt = newest_mtime(hb, glob=glob)
        freshness_basis = "mtime"
        if date_stamped:
            # 日期戳产线: 真实新鲜度取文件名作者日期, 防 fs mtime 被 git/touch 刷新而掩盖冻结。
            # 取 文件名日期 与 mtime 二者中**更旧**者作为新鲜度依据(谁更能暴露停更用谁),
            # 这样 06-22 一次无关 git commit 把 mtime 刷新也无法把真冻结洗成假新鲜。
            fn_date = newest_filename_date(hb, glob=glob)
            if fn_date is not None:
                mt = fn_date if (mt is None or fn_date < mt) else mt
                freshness_basis = "filename_date"
        if mt is None:
