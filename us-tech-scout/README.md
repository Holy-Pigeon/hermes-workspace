...
## 边界 / 待办
- **护城河判定已收口到 `../moat-durability/moat_core.py` 单一事实源**(2026-06-15): 耐久度门槛/
  定价权护栏/侵蚀红旗/低谷穿越升级与 moat_scorecard **完全同源**, 不再各写一份防漂移。本工具
  在其上叠加「成长维度」做发现漏斗(🏰=宽护城河+在成长 / ⭐=近门槛+在成长 / 🔍=宽护城河但停滞 /
  护城河存疑(净利率收缩/ROE不持久)直接不入候选)。**口径变化**: 统一后 🏰 用 moat 的 CV≤0.20
  (比旧版 CV≤0.35 严), MSFT 这类 CV0.212 的会落 ⭐ 而非 🏰(仍是候选), 换来与 A 股工具一致的
  护城河标准 + 美股名首次具备侵蚀/定价权护栏。
