#!/usr/bin/env python3
"""Sync an Agent-Notes Markdown entry into a dedicated Notion database."""
...
CONFIG_PATH = ROOT / ".openclaw" / "notion_agent_notes.json"
...
def ensure_database(token: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    title = config.get("database_title") or "Agent 经验沉淀"
...
def create_lesson_page(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.maturity:
        properties["Maturity"] = {"select": {"name": args.maturity[:100]}}
