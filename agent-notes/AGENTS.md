...
- **Doc Type**（文章类型，单选）：`分析文` / `经验文` —— 体裁维度，与 Category（主题）正交。**分析文**=我基于源码/二进制一手取证推导出的机制长文；**经验文**=用户口述、我润色的实战踩坑心得。一篇只属一种体裁。
- **Tags**（多选标签）：一条经验常跨多个点，自由打标
- **Maturity**（成熟度，单选）：`验证过` / `经验法则` / `待验证` —— 体现内容可信度。**注意**：成熟度 ≠ 体裁，「分析文/经验文」走 Doc Type，别再往 Maturity 里塞「分析文档」这种体裁值。
