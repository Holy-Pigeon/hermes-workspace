...
DIRECTION_STANCE = {
    ...
    "diversification": 0,
    "neutral": 0,
    # us-tech-scout 暗区 sleeve 的呼叫标签(candidate-deepening cron 产出)
    "down": -1,
    "bullish_on_growth": +1,
    "bullish_on_margin": +1,
}

# 关键词兜底: candidate-deepening cron 会写自由文本 direction(含中文整句),
# 硬编码枚举必然漂移。这里做关键词归一, 让新标签能被计分而非静默丢进 UNKNOWN_DIR。
# 净空/净多冲突时以更强信号词判定; 命中不了才真 UNKNOWN(并在 --quiet 下也报警)。
_BEAR_KW = ("看空", "bearish", "overvalued", "value_trap", "distribution",
            "topping", "short", "派发", "高估", "减持", "down", "avoid")
_BULL_KW = ("看多", "bullish", "undervalued", "real_cheap", "mispriced_trough",
            "organic_ramp", "recovery", "inflection", "accumulation",
            "moat_understated", "低估", "便宜", "吸筹", "拐点", "up", "buy")
_NEUTRAL_KW = ("diversif", "correlation", "neutral", "分散", "对冲", "低相关")


def resolve_stance(direction):
    """先查显式枚举, 再关键词兜底。返回 (+1/-1/0/None, 是否走了兜底)。"""
    if direction in DIRECTION_STANCE:
        return DIRECTION_STANCE[direction], False
    d = (direction or "").lower()
    if any(k in d for k in _NEUTRAL_KW):
        return 0, True
    is_bear = any(k in d for k in _BEAR_KW)
    is_bull = any(k in d for k in _BULL_KW)
    if is_bear and not is_bull:
        return -1, True
    if is_bull and not is_bear:
        return +1, True
    # 同时命中或都没命中 → 交给人工核实, 不猜方向(避免把方向判反)
    return None, True
...
def fetch_stock_series(symbol, market):
    ...
    digits = "".join(ch for ch in symbol if ch.isdigit())
    if market == "US":
        # US subject 常是显示串 "Apple(AAPL)"/"ASML Holding (ASML)", 须抽括号内 ticker;
        # 无括号则取末段纯字母 token。否则整串传给 get_daily 必失败(此前 P014-P020 全 SKIP)。
        m = re.search(r"\(([A-Za-z.\-]{1,6})\)", symbol)
        if m:
            code = m.group(1).upper()
        else:
            toks = re.findall(r"[A-Za-z.\-]{1,6}", symbol)
            code = (toks[-1].upper() if toks else symbol)
    else:
...
def main():
    ...
    for p in preds:
        ...
        stance, via_keyword = resolve_stance(direction)
    ...
    unknown = [r for r in rows if r["status"] == "UNKNOWN_DIR"]
    # --quiet 下: 无可计分呼叫且无未映射方向标签才静默。
    # UNKNOWN_DIR = 生产端(candidate-deepening)写了消费端识别不了的 direction,
    # 属计分链漏计的契约漂移, 必须浮现而非静默吞掉(否则新 sleeve 永远测不到)。
    if quiet and not ok and not unknown:
        sys.exit(0)
    ...
    if ok:
        ...
        for r in sorted(ok, key=lambda x: -x["call_alpha"]):
            ...
            kw = "*" if r.get("via_keyword") else ""
            print(f"{flag} {r['id']}{kw} {r['subject'][:18]:<18} [{r['stance']}] "
                  f"自{r['from']}: 标的{r['stock_ret']:+.1f}% "
                  f"vs {r['bench']}{r['bench_ret']:+.1f}% "
                  f"→ 呼叫α {r['call_alpha']:+.1f}pp")
        if any(r.get("via_keyword") for r in ok):
            print("  (* = direction 由关键词兜底归一, 非显式枚举; 建议回填枚举)")
